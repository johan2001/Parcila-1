...
using namespace std;
bool comparar_string(string usuario,string contrasena,string _usuario,string _contrasena)// compara dos palabras con otras dos palabras
{
...
bool comparar_usuario_clave(map<string,string> usuarios_contrasenas,string _usuario,string _contrasena)
{
    ...
    for(auto par= begin(usuarios_contrasenas); par != end(usuarios_contrasenas); par++){
         ...
         des= comparar_string(usuario,contrasena,_usuario,_contrasena);
         if(des){return true;}
    }
    return false;
}

void estraer_datos_de_usuarios(string*);
void estraer_inventario();
void guardar_datos(map<int,vector<string>> usuarios_contrasenas , string usuario,string contrasena )
{
